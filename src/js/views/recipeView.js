...
const createIngredientMarkup = (ingredient) => {
  return `
    <li class="recipe__item">
      ...
      <div class="recipe__count">${ formatRecipeCount(ingredient.count)}</div>
  ...
  const markup = `
    ...
    <div class="recipe__ingredients">
      <ul class="recipe__ingredient-list">
        ${recipe.ingredients.map( el => createIngredientMarkup(el) ).join(' ')}
      </ul>
...
export const updateServingsIngredients = recipe => {
  ...
  countElements.forEach( (el, i) => {
    el.textContent = formatRecipeCount(recipe.ingredients[i].count);
  })
}